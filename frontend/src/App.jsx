...
export default function App() {
  ...
  const tablePlayers = (state.players || []).map((p) => (
    p.is_me && state.me?.role
      ? { ...p, role: state.me.role, role_name: state.me.role_name, team: state.me.team, image: state.me.image }
      : p
  ));
  const showMyRoleCard = ["night", "morning", "discussion", "voting", "day_result"].includes(state.phase);
    ...
    <div className={"app app-shell phase-" + theme}>
      ...
      <main className="main-content content with-sidebar">
        ...
        {["night", "morning", "discussion", "voting", "day_result"].includes(state.phase) && (
          <>
            {showMyRoleCard && <MyRoleCard me={state.me} />}
            <MafiaTable
